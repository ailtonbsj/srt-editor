(() => {
  ...
  const cpsLimitInput = $('#cpsLimitInput');
  const autoMergeBtn = $('#autoMergeBtn');
  ...
  /* ============ Auto-merge fast cues ============
     For every cue whose CPS exceeds the given limit, merge it with
     whichever neighbor (previous or next) produces the better result:
       1. Prefer the neighbor whose merged CPS ends up lower (i.e. actually
          helps, rather than just moving the problem around).
       2. If both directions are roughly equal, prefer the neighbor with
          the smaller time gap — merging across a big silent gap creates
          an oddly-timed cue, while merging across a short gap reads
          naturally as one continuous thought.
     Cues with no eligible neighbor (e.g. the only cue in the file) are
     left flagged and reported back as "remaining". */
  function simulateMerge(a, b){
    const text = [a.text, b.text].filter(Boolean).join(' ').replace(/\n/g, ' ');
    const duration = Math.max(0, b.end - a.start);
    const length = text.length;
    const cps = duration > 0 ? length / duration : Infinity;
    return { cps, duration };
  }

  function autoMergeFastCues(limit){
    let mergedCount = 0;
    const maxIterations = cues.length * 3 + 10; // safety net against pathological loops
    let iterations = 0;
    let progress = true;
    ...
    while (progress && iterations < maxIterations){
      ...
      for (let i = 0; i < cues.length; i++){
        const cps = metricsFor(cues[i]).cps;
        if (!isFinite(cps) || cps <= limit) continue;

        const hasPrev = i > 0;
        const hasNext = i < cues.length - 1;
        if (!hasPrev && !hasNext) continue; // nothing to merge with

        let direction;
        if (hasPrev && hasNext){
          const prevSim = simulateMerge(cues[i - 1], cues[i]);
          const nextSim = simulateMerge(cues[i], cues[i + 1]);
          const EPS = 0.05; // cps tolerance to treat two options as "roughly equal"
          if (prevSim.cps < nextSim.cps - EPS){
            direction = 'prev';
          } else if (nextSim.cps < prevSim.cps - EPS){
            direction = 'next';
          } else {
            const prevGap = cues[i].start - cues[i - 1].end;
            const nextGap = cues[i + 1].start - cues[i].end;
            direction = prevGap <= nextGap ? 'prev' : 'next';
          }
        } else {
          direction = hasPrev ? 'prev' : 'next';
        }

        if (direction === 'prev') mergeCues(i - 1, i);
        else mergeCues(i, i + 1);

        mergedCount++;
        progress = true;
        break; // cue indices shifted — restart the scan
      }
    }

    const remaining = cues.reduce((n, c) => {
      const cps = metricsFor(c).cps;
      return n + (isFinite(cps) && cps > limit ? 1 : 0);
    }, 0);

    return { merged: mergedCount, remaining };
  }
  ...
  autoMergeBtn.addEventListener('click', () => {
    if (!cues.length){ toast('Nothing to merge yet.'); return; }
    const limit = parseFloat(cpsLimitInput.value);
    if (!isFinite(limit) || limit <= 0){
      toast('Enter a valid CPS limit first.');
      cpsLimitInput.focus();
      return;
    }
    closeAllMenus();
    const { merged, remaining } = autoMergeFastCues(limit);
    renderCues();
    if (merged === 0){
      toast(`No cues above ${limit} cps — nothing to merge.`);
    } else if (remaining === 0){
      toast(`Merged ${merged} cue${merged === 1 ? '' : 's'} — every cue is now at or under ${limit} cps.`);
    } else {
      toast(`Merged ${merged} cue${merged === 1 ? '' : 's'} — ${remaining} still above ${limit} cps (no usable neighbor).`);
    }
  });
